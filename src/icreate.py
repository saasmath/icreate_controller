...
class iCreate:
  ...
  def __init__(self,sensorCallBack=None,imageCallBack=None,imageTopic="/camera/image_raw"): 
    ...
    self._shutdownservice = rospy.Service('icreate_shutdown', iCreateShutdown, self.iCreateShutdown)
  ...
  #================================  
  def iCreateShutdown(self,reason):
    """
  ...
  def turnAngle(self,angle,speed=50): 
    ...
    duration = abs(angle*.02295*100.0/abs(speed))
...
#================================
def colorblobs(img,colors,maxChannelThreshold,showVideo=False,showDebug=False):
  """
    ...
    {'color':[array of blob rectangle bounds in form ((x1,y1),(x2,y2),(center of blob x, y),area of blob)}
      ...
      maxChannelThreshold-maximum threshold that a single channel has to differ from input color
  ...
  """
  thres = maxChannelThreshold #maximum channel error for color
  ...
  div = max(1, int((cols*rows)/4800)) #use 1/div amount of pixels in image
      ...
      for k,v in filled.iteritems():
        ...
        for p in v:
          ...
          if(p[1]>ay):
            ...
            iy = p[1]
          cv.Circle(img, (p[0],p[1]), 4, (colors[k][0]/2,colors[k][1]/2,colors[k][2]/2))   
    #show blobs on screen
    for k,v in colorblobs.iteritems():
      for b in v:
        cv.Rectangle(img, b[0], b[1], (colors[k][0],colors[k][1],colors[k][2]))
